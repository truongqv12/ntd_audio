...
import type { HealthResponse, Project, ProviderCredential, ProviderSummary, SettingsOverview } from "../types";
...
function cloneCredentialFields(credential: ProviderCredential) {
  return Object.fromEntries(Object.entries(credential.fields).map(([key, field]) => [key, field.value ?? ""]));
}
...
export const SettingsPage = memo(function SettingsPage({
  ...
  const providerMap = useMemo(() => new Map(providers.map((provider) => [provider.key, provider])), [providers]);
  ...
  useEffect(() => {
    ...
    setProjectDraft({
      ...
      merge_silence_ms: Number(projectSettings.merge_silence_ms ?? settingsOverview?.merge_defaults?.merge_silence_ms ?? 150),
      merge_output_format: String(projectSettings.merge_output_format ?? settingsOverview?.merge_defaults?.merge_output_format ?? "wav"),
    });
  ...
  return (
    <div className="settings-page-grid">
      <Panel title="Provider credentials" description="Set API keys and runtime endpoints used by cloud and self-hosted providers. Environment variables still take precedence when present.">
              ...
              <div className="provider-settings-card" key={credential.provider_key}>
                <div className="provider-stack-head">
                  <div>
                    ...
                    <p>{credential.provider_key} · {credential.category}</p>
                  </div>
                  <span className={`status-tag ${credential.configured ? "status-tag-succeeded" : "status-tag-queued"}`}>
                    ...
                    <label className="form-field compact" key={fieldKey}>
                      ...
                      <input
                        ...
                        onChange={(event) => setCredentialDrafts((current) => ({
                          ...current,
                          [credential.provider_key]: { ...(current[credential.provider_key] ?? {}), [fieldKey]: event.target.value },
                        }))}
                      />
                      {field.env_present ? <small className="muted-copy">ENV override active: {field.env}</small> : null}
                    </label>
                  ))}
                </div>
                <button type="button" className="ghost-button compact-button" onClick={() => void saveCredential(credential.provider_key)} disabled={savingProvider === credential.provider_key}>
      ...
      <Panel title="Project defaults" description="Defaults applied to Create Job and Script Editor rows unless a row overrides them.">
        <div className="filter-inline-grid">
          ...
          <div className="form-field compact">
            ...
            <select value={String(projectDraft.default_provider_key ?? "")} onChange={(event) => setProjectDraft((current) => ({ ...current, default_provider_key: event.target.value }))}>
          ...
          <div className="form-field compact">
            ...
            <select value={String(projectDraft.default_output_format ?? "mp3")} onChange={(event) => setProjectDraft((current) => ({ ...current, default_output_format: event.target.value }))}>
        ...
        <div className="filter-inline-grid">
          <div className="form-field compact">
            ...
            <input type="number" min={0} value={Number(projectDraft.merge_silence_ms ?? 150)} onChange={(event) => setProjectDraft((current) => ({ ...current, merge_silence_ms: Number(event.target.value) }))} />
          </div>
          <div className="form-field compact">
            <label>Merge output</label>
            <select value={String(projectDraft.merge_output_format ?? "wav")} onChange={(event) => setProjectDraft((current) => ({ ...current, merge_output_format: event.target.value }))}>
              <option value="wav">wav</option>
              <option value="mp3">mp3</option>
            </select>
          </div>
        </div>
        <button className="primary-button compact-primary" type="button" onClick={() => void saveProjectDefaults()} disabled={!currentProject}>Save project defaults</button>
      </Panel>

      <Panel title="Global merge defaults" description="Fallback values used when a project has not customized merge behavior.">
        <div className="filter-inline-grid">
          <div className="form-field compact">
            <label>Merge silence ms</label>
            <input type="number" min={0} value={Number(mergeDraft.merge_silence_ms ?? 150)} onChange={(event) => setMergeDraft((current) => ({ ...current, merge_silence_ms: Number(event.target.value) }))} />
          </div>
          <div className="form-field compact">
            <label>Merge output format</label>
            <select value={String(mergeDraft.merge_output_format ?? "wav")} onChange={(event) => setMergeDraft((current) => ({ ...current, merge_output_format: event.target.value }))}>
              <option value="wav">wav</option>
              <option value="mp3">mp3</option>
            </select>
          </div>
          <div className="form-field compact">
            ...
            <select value={String(mergeDraft.normalize_loudness ?? false)} onChange={(event) => setMergeDraft((current) => ({ ...current, normalize_loudness: event.target.value === "true" }))}>
              <option value="false">Off</option>
              <option value="true">On</option>
            </select>
          </div>
        </div>
        <button className="ghost-button compact-button" type="button" onClick={() => void saveMergeDefaults()}>Save global merge defaults</button>
      </Panel>

      <Panel title="Voice parameter schemas" description="Parameter forms are generated from provider schemas so every engine can expose different controls without hard-coding UI per voice.">
        <div className="schema-list">
          {Object.entries(schemas).map(([providerKey, fields]) => (
            <details key={providerKey} className="schema-details">
              <summary>{providerKey} · {fields.length} fields</summary>
              <div className="settings-stack">
                {fields.map((field) => (
                  <div className="settings-item" key={field.key}>
                    <span>{field.label}</span>
                    <strong>{field.key} · {field.kind}{field.unit ? ` · ${field.unit}` : ""}</strong>
                  </div>
                ))}
              </div>
            </details>
