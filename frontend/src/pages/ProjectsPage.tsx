    ...
    <div className="page-grid two-up">
      <Panel title={t("projectsPage.title")} description={t("projectsPage.description")}>
        <div className="project-grid">
          {projects.map((project) => (
            <div
              className={`project-card ${selectedProjectKey === project.project_key ? "project-card-active" : ""}`}
            ...
            >
              ...
              <div className="mini-metrics">
                <span>
                  {project.stats.total_jobs} {t("projectsPage.jobs")}
                </span>
                <span>
                  {project.stats.succeeded_jobs} {t("projectsPage.ok")}
                </span>
                <span>
                  {project.stats.failed_jobs} {t("projectsPage.fail")}
                </span>
              </div>
      ...
      <Panel title={t("projectsPage.createTitle")} description={t("projectsPage.createDescription")}>
        <div className="form-field">
          <label>{t("projectsPage.projectKey")}</label>
          <input
            value={form.project_key}
            onChange={(e) => setForm((current) => ({ ...current, project_key: e.target.value }))}
            placeholder={t("projectsPage.placeholderKey")}
          />
        </div>
        <div className="form-field">
        ...
        <div className="form-field">
          <label>{t("projectsPage.descriptionLabel")}</label>
          <textarea
            rows={4}
        ...
        <div className="filter-inline-grid">
          <div className="form-field compact">
            ...
            <select
              value={form.default_provider_key}
              onChange={(e) => setForm((current) => ({ ...current, default_provider_key: e.target.value }))}
            >
          ...
          <div className="form-field compact">
            ...
            <select
              ...
              onChange={(e) => setForm((current) => ({ ...current, default_output_format: e.target.value }))}
            >
        ...
        <div className="form-field">
          <label>{t("projectsPage.tags")}</label>
          <input
            value={form.tags}
            onChange={(e) => setForm((current) => ({ ...current, tags: e.target.value }))}
            placeholder={t("projectsPage.placeholderTags")}
          />
        </div>
        <button
          type="button"
