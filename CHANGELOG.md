...
- **`/v1/settings/voice-parameter-schemas` returned 500 under pydantic 2.13.** Several `ProviderParamField` entries (`piper.speaker_id`, `voicevox.{prePhonemeLength,postPhonemeLength}`, `google_cloud_tts.{sampleRateHertz,lowLatencyJourneySynthesis}`, `azure_speech.style`, `vieneu_tts.{reference_audio_path,reference_text}`, and the shared `gain_db` post-param) passed `True` as the 10th positional argument intending `advanced=True`, but position 10 in the dataclass is `options: list[dict[str, str]]`. Older pydantic silently coerced; 2.13 (pulled in by the `pydantic-settings>=2.6` bump from the previous fix) is strict and rejects `True` as a list. Switched all of these to keyword-only `advanced=True`. Added `tests/test_providers_params.py` to round-trip every entry through `ProviderParameterSchemasResponse` so future positional drift fails CI immediately.
