...
from .schemas import ProjectBatchQueueResponse, ProjectMergeResponse, ProjectRowsResponse, QueueProjectRowsRequest, UpsertProjectRowsRequest
from .services_project_rows import list_project_rows, merge_project_rows, project_row_artifact_path, queue_project_rows, replace_project_rows
...
def put_project_rows(project_key: str, payload: UpsertProjectRowsRequest, db: Session = Depends(get_db)) -> ProjectRowsResponse:
    return replace_project_rows(db, project_key, payload)


@router.post("/queue", response_model=ProjectBatchQueueResponse)
def queue_rows(project_key: str, payload: QueueProjectRowsRequest, db: Session = Depends(get_db)) -> ProjectBatchQueueResponse:
...
def merge_rows(project_key: str, payload: QueueProjectRowsRequest, db: Session = Depends(get_db)) -> ProjectMergeResponse:
