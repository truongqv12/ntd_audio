...
from .models import GenerationCache, JobEvent, Project, ProjectScriptRow, SynthesisArtifact, SynthesisJob, VoiceCatalogEntry
from .provider_registry import get_provider
from .services_app_settings import apply_provider_settings
from .schemas import ArtifactResponse, CreateJobRequest, JobEventResponse, JobResponse, JobsResponse, LiveSnapshotResponse
...
def _build_cache_key(provider_key: str, provider_voice_id: str, source_text: str, output_format: str, params: dict) -> str:
...
def create_job(db: Session, payload: CreateJobRequest) -> JobResponse:
    ...
    cache_key = _build_cache_key(payload.provider_key, payload.provider_voice_id, payload.source_text, payload.output_format, payload.params)
    job = SynthesisJob(
    ...
    db.add(job)
    db.flush()
    db.add(JobEvent(job_id=job.id, event_type="queued", message="Job queued", payload={"project_key": project.project_key}))
    db.commit()
    db.refresh(job)
    logger.info("job_created job_id=%s project=%s provider=%s voice=%s", job.id, project.project_key, job.provider_key, job.provider_voice_id)
...
def process_job(db: Session, job_id: str) -> None:
    ...
    try:
        ...
        db.add(artifact)
        db.add(cache_row)
        db.add(JobEvent(job_id=job.id, event_type="completed", message="Synthesis completed", payload=result.provider_metadata))
        db.commit()
        logger.info("job_completed job_id=%s provider=%s artifact=%s size=%s", job.id, job.provider_key, relative_path, file_size)
